...
def set_server(ip, target_location, port=22):
    config.server_ip = ip
    config.server_location = target_location
...
def connect():
    ...
    ssh.connect(config.server_ip, config.server_port, config.user, config.password)
    print(f'\t[remote] connected to {config.user}@{config.server_ip}:{config.server_port}')
    return ssh
...
def upload(local_file, remote_file):
    ...
    if ssh is None:
        ssh = connect()
    exec_command(f'mkdir {config.server_location}')
    sftp = ssh.open_sftp()
